...
def main():
    ...
    accuracies_dv = np.zeros(len(train_sizes))
    accuracies_tr = np.zeros(len(train_sizes))

    for run, size in enumerate(train_sizes):

        X_train_run = X_train[:size]
        y_train_run = y_train[:size]

        #print(X_train_run.shape, y_train_run.shape)
        #print(X_dev.shape, y_dev.shape)

         # Create bias nodes
        bias_tr = np.ones((X_train_run.shape[0], 1))
        bias_dv = np.ones((X_dev.shape[0], 1))
        # Add bias nodes to input
        X_train_run = np.hstack((X_train_run, bias_tr))
        X_dev_run = np.hstack((X_dev, bias_dv))

        accuracy_tr = 0; accuracy_dv = 0
        # Reset model
        model = init_weights(n_feature=n_feature, n_class=n_class)

        # Train the model
        model = gradient_descent(model, X_train_run, y_train_run,
                                 n_class, no_iter=no_iter)


        # Predict dev data
        y_pred = np.zeros_like(y_dev)
        for i, x in enumerate(X_dev_run):
        ...
        # Predict train data
        y_pred_tr = np.zeros_like(y_train_run)
        for i, x in enumerate(X_train_run):
            # Predict the distribution of label
            _, _, prob = forward(x, model)
            # Get label by picking the most probable one
            y = np.argmax(prob)
        ...
        accuracy_dv = (y_pred == y_dev).sum() / y_dev.size
        accuracy_tr = (y_pred_tr == y_train_run).sum() / y_train_run.size

        #print('Accuracy after {} iterations: {}'.format(no_iter,accuracy))
        #pylab.scatter(X_test[:,0], X_test[:,1], c=y_pred)
        #pylab.show()
        accuracies_tr[run] = accuracy_tr
        accuracies_dv[run] = accuracy_dv

        print(run, size, accuracy_tr, accuracy_dv)
